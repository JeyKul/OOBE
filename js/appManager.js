...
(function (CloudExperienceHost) {
    class AppManager {
        ...
        _setScenario(args) {
            ...
            if (!scenario || (scenario.length === 0)) {
                if (CloudExperienceHost.FeatureStaging.isOobeFeatureEnabled("DoNotCrashCxhWhenNoArgs")) {
                    ...
                    // Since we are trying to terminate the application while still within the "onactivated" handler, we need
                    // to stop the application first to prevent triggering the E_ACTIVATION_APPLICATION_TERMINATED error.
                    ...
                    if (CloudExperienceHost.FeatureStaging.isOobeFeatureEnabled("OobeInEnduserSession")) {
                        if (this._navigator.getNavMesh().getWaitForTaskbarReady()) {
                            CloudExperienceHostAPI.Synchronization.waitForTaskbarReadyAsync(30000 /* timeoutMs */).then(() => {
                                ...
                                // Send signal to crossfade to desktop. If we fail to crossfade, fallback is "hard cut" to desktop.
                                Windows.UI.ViewManagement.ApplicationView.getForCurrentView().tryConsolidateAsync().then((success) => {
                                    ...
                                    if (!success) {
                                        ...
                                        // Force cxh to exit upon timeout in case cxh couldn't be closed by EM for some reason like explorer being crash.
                                        setTimeout(() => {
        ...
        _closeWindowAndTerminateApp() {
            window.close();
            // This code ensures the wwahost.exe host process for CloudExperienceHost is always terminated once there is no longer any visible UI.
            // This is required following a change in HAM that no longer guarantees this.
